...
class Client {
  ...
  allcategory = {
    all: () => HttpClient.get<Category>(API_ENDPOINTS.CATEGORIES),
    
  };

  allsubcategory = {
    all: () => HttpClient.get<Subcategory>(API_ENDPOINTS.SUBCATEGORIES),
    find: () => {
      return HttpClient.get<Subcategory[]>(API_ENDPOINTS.SUBCATEGORIES);
    },
  };

  allbanner = {
    all: () => HttpClient.get<Banner>(API_ENDPOINTS.BANNERS),
    find: () => {
      return HttpClient.get<Banner[]>(API_ENDPOINTS.BANNERS);
    },
  };
